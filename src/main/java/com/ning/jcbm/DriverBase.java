...
public abstract class DriverBase extends JapexDriverBase {
	/*
	 * 24-Mar-2011, tatu: Looks like we get skewed values for some things;
	 * specifically, comp/uncomp speeds in gigabytes. This is problematic mostly
	 * just for graphs, since outliers stretch stuff. So, let's just cap to some
	 * max value.
	 * 
	 * For compression, 0.5 gig/sec could serve as boundary (highest so far seen
	 * was 2.5G/s); no compressible test case has produced such speeds for
	 * compression. For decompression, fastest codecs can legitimately expand at
	 * bit higher speeds, so let's upgrade this to 0.6 gig/sec for now
	 */
	// 07-Jun-2013, tatu: Time flies, performance climbs. Need higher limits

	final static double MAX_COMPRESS_THROUGHPUT = 6999.9;

	final static double MAX_UNCOMPRESS_THROUGHPUT = 14999.9;

	final static double MAX_BOTH_THROUGHPUT = 11699.9;
	...
	/**
	 * Directory in which input files are stored (file names are same as test
	 * case names)
	 */
	...
	public void prepare(TestCase testCase) {
		...
		if (parts.length < 2) {
			throw new IllegalArgumentException(
					"Invalid test name '" + name + "'; should have at least 2 components separated by slash");
		}
		_operation = null;
		...
		if (operStr.startsWith("C")) {
			...
			throw new IllegalArgumentException(
					"Invalid 'operation' part of name, '" + operStr + "': should start with 'C', 'U' or 'R'");
		}
		_inputFile = new File(_inputDir, filename);
		try {
			// First things first: load uncompressed input in memory; compress
			// to verify round-tripping
			_uncompressed = loadFile(_inputFile);
			// _uncompressedBB = ByteBuffer.wrap(_uncompressed);
			_uncompressedBB = ByteBuffer.allocateDirect(_uncompressed.length);
			_uncompressedBB.put(_uncompressed);
			...
			// _compressedBB = ByteBuffer.wrap(_compressed);
			_compressedBB = ByteBuffer.allocateDirect(_compressed.length);
			_compressedBB.put(_compressed);
			_compressBuffer = new byte[maxCompressedLength(_uncompressed.length)];
			// _compressBufferBB = ByteBuffer.wrap(_compressBuffer);
			_compressBufferBB = ByteBuffer.allocateDirect(_compressBuffer.length);
			_compressBufferBB.put(_compressBuffer);
			// +1024 so that it can't be used by the driver to find the original
			// length
			_uncompressBuffer = new byte[_uncompressed.length + 1024];
			// _uncompressBufferBB = ByteBuffer.wrap(_uncompressBuffer);
			_uncompressBufferBB = ByteBuffer.allocateDirect(_uncompressBuffer.length);
			_uncompressBufferBB.put(_uncompressBuffer);
	...
	public void finish(TestCase testCase) {
		...
		double itersPerSec = 1000.0 * testCase.getDoubleParam(Constants.RUN_ITERATIONS_SUM)
				/ testCase.getDoubleParam(Constants.ACTUAL_RUN_TIME);
	...
	protected abstract void compressToStream(byte[] uncompressed, OutputStream out) throws IOException;

	protected abstract int uncompressFromStream(InputStream in, byte[] buffer) throws IOException;
	...
	protected void verifyRoundTrip(String name, byte[] raw, byte[] compressed) throws IOException {
		...
		if (actualLength != raw.length) {
			throw new IllegalArgumentException("Round-trip failed for driver '" + _driverName + "', input '" + name
					+ "': uncompressed length was " + actual.length + " bytes; expected " + raw.length);
		}
		int i = 0;
		for (int len = raw.length; i < len; ++i) {
			if (actual[i] != raw[i]) {
				throw new IllegalArgumentException("Round-trip failed for driver '" + _driverName + "', input '" + name
						+ "': bytes at offset " + i + " (from total of " + len + ") differed, expected 0x"
						+ Integer.toHexString(raw[i] & 0xFF) + ", got 0x" + Integer.toHexString(actual[i] & 0xFF));
			}
		}
	}
